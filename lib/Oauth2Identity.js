...
/** @typedef {import('@advanced-rest-client/events').Authorization.BaseOAuth2Authorization} BaseOAuth2Authorization */
...
export class Oauth2Identity {
   ...
   */
  static listen() {
    ipcMain.handle('oauth2', Oauth2Identity.proxyOauthAction);
    ipcMain.handle('oidc', Oauth2Identity.proxyOidcAction);
  }
   ...
   */
  static unlisten() {
    ipcMain.removeHandler('oauth2');
    ipcMain.removeHandler('oidc');
  }

  /**
   * @param {Electron.IpcMainEvent} e
   * @param {string} action The action to perform.
   * @param {unknown} options Passed in the renderer process options.
   */
  static async proxyOauthAction(e, action, options) {
    switch (action) {
      case 'getToken': return Oauth2Identity.getAuthToken(/** @type BaseOAuth2Authorization */ (options));
      case 'launchWebFlow': return Oauth2Identity.launchWebAuthFlow(/** @type OAuth2Authorization */ (options));
      case 'removeToken': return Oauth2Identity.removeToken(/** @type TokenRemoveOptions */ (options));
      default: throw new Error(`Unknown OAuth2 action: ${action}`);
    }
  }
   ...
   * @param {unknown} options Passed in the renderer process options.
   */
  static async proxyOidcAction(e, action, options) {
    switch (action) {
      case 'getToken': return Oauth2Identity.getOidcTokens(/** @type OidcAuthorization */ (options));
      case 'removeTokens': return Oauth2Identity.removeOidcToken(/** @type TokenRemoveOptions */ (options));
      default: throw new Error(`Unknown OIDC action: ${action}`);
    }
  }
